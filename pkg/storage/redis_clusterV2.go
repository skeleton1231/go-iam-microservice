...
func (r *RedisClusterV2) DeleteRawKey(keyName string) bool {
	if err := r.up(); err != nil {
		return false
	}
...
func (r *RedisClusterV2) DeleteScanMatch(pattern string) bool {
	if err := r.up(); err != nil {
		return false
	}
	client := r.singleton()
	...
	if len(keys) > 0 {
		...
		log.Debug("RedisClusterV2 called DEL - Nothing to delete")
	}
...
func (r *RedisClusterV2) DeleteKeys(keys []string) bool {
	if err := r.up(); err != nil {
		return false
	}
	if len(keys) > 0 {
...
func (r *RedisClusterV2) StartPubSubHandler(channel string, callback func(interface{})) error {
	if err := r.up(); err != nil {
		return err
	}
	client := r.singleton()
...
func (r *RedisClusterV2) Publish(channel, message string) error {
	if err := r.up(); err != nil {
		return err
	}
...
func (r *RedisClusterV2) GetAndDeleteSet(keyName string) []interface{} {
...
func (r *RedisClusterV2) AppendToSet(keyName, value string) {
	fixedKey := r.fixKey(keyName)
...
func (r *RedisClusterV2) Exists(keyName string) (bool, error) {
	fixedKey := r.fixKey(keyName)
...
func (r *RedisClusterV2) RemoveFromList(keyName, value string) error {
	fixedKey := r.fixKey(keyName)
...
func (r *RedisClusterV2) GetListRange(keyName string, from, to int64) ([]string, error) {
	fixedKey := r.fixKey(keyName)
...
func (r *RedisClusterV2) AppendToSetPipelined(key string, values [][]byte) {
	if len(values) == 0 {
		return
	}
...
func (r *RedisClusterV2) GetSet(keyName string) (map[string]string, error) {
...
func (r *RedisClusterV2) AddToSet(keyName, value string) {
	log.Debugf("Pushing to raw key set: %s", keyName)
	log.Debugf("Pushing to fixed key set: %s", r.fixKey(keyName))
	if err := r.up(); err != nil {
		return
	}
	err := r.singleton().SAdd(r.Ctx, r.fixKey(keyName), value).Err()
	if err != nil {
...
func (r *RedisClusterV2) RemoveFromSet(keyName, value string) {
...
func (r *RedisClusterV2) IsMemberOfSet(keyName, value string) bool {
	if err := r.up(); err != nil {
		log.Debug(err.Error())
...
func (r RedisClusterV2) GetRollingWindow(keyName string, per int64, pipeline bool) (int, []interface{}) {
	if err := r.up(); err != nil {
		log.Debug(err.Error())
...
func (r *RedisClusterV2) AddToSortedSet(keyName, value string, score float64) {
	fixedKey := r.fixKey(keyName)
	...
	if err := r.singleton().ZAdd(r.Ctx, fixedKey, member).Err(); err != nil {
		log.Error(
			"ZADD command failed",
			log.String("keyName", keyName),
			log.String("fixedKey", fixedKey),
...
func (r *RedisClusterV2) GetSortedSetRange(keyName, scoreFrom, scoreTo string) ([]string, []float64, error) {
	fixedKey := r.fixKey(keyName)
	log.Debug(
...
func (r *RedisClusterV2) RemoveSortedSetRange(keyName, scoreFrom, scoreTo string) error {
	fixedKey := r.fixKey(keyName)
